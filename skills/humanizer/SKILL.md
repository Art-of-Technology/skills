...
name: humanizer
description: |
  Remove signs of AI-generated writing from CUSTOMER-FACING content only: email
  subject lines and bodies, web UI copy (headings, buttons, empty states, error
  and toast messages, onboarding), landing/marketing pages, in-app notifications,
  changelogs and release notes shown to users, and support replies. Run it on
  that content before it ships. Do NOT apply to code, code comments, commit
  messages, PR descriptions, internal docs, or chat replies to the developer.
...
## Scope (Art of Technology fork)

Apply this skill only to text a customer or end user will read: email subject
and body, web UI strings, marketing pages, in-app notifications, user-facing
changelog entries, support replies. Skip everything internal: code, comments,
commit messages, PR text, internal docs, developer chat.

For UI strings, keep it short and keep the meaning identical. Do not change the
action a button performs, the placeholder a field expects, or the key in an i18n
file; only the human-readable value.

Upstream: https://github.com/blader/humanizer (MIT, see LICENSE).
